...
export default function AuthProvider({
    ...
    const loadSession = useCallback(async (route?: string) => {
        ...
        if (typeof window !== 'undefined' && window.location.pathname === '/signin-oidc') {
            setLoading(false);
            loadingRef.current = false;
            return;
        }

        // If a logout is in progress, skip all auth checks — the signOut flow
        // will redirect via window.location, no router navigation needed here.
        // This prevents a Safari-specific infinite loop where router.replace('/login')
        // triggers an RSC fetch that Safari blocks (ITP / cross-origin context after
        // the Cognito redirect), causing Next.js to fall back to a full page reload
        // which re-mounts AuthProvider and repeats the cycle endlessly.
        ...
        try {
            ...
            if (context.user.isAdmin || context.user.isTechnician) {
                ...
                errorToast({
                    title: 'Forbidden',
                    description: "You don't have the right permission to access the company",
                });
    ...
    }, [saveUserDetail, clearLocalSession]); // router intentionally excluded — using routerRef
    ...
    const refreshSession = useCallback(async (route?: string) => {
