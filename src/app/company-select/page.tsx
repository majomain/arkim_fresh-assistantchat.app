    ...
    useEffect(() => {
        const fetchCompanies = async () => {
            try {
                ...
                setCompanies(companiesList.filter((company) => company.isAdmin || company.isTechnician));
        ...
        setSelectedCompanyId(localStorage.getItem(STORAGE_KEYS.SELECTED_COMPANY_ID) ?? null);
        ...
        return (
            <div className="fixed inset-0 flex items-center justify-center z-50" style={{
                ...
                <div className='flex flex-col gap-1'>
                    ...
                    <p className="text-sm text-muted-foreground text-center">Loading company list...</p>
                </div>
            </div>
        );
    }
                ...
                isLoggingOut
                    ...
                    <Card className="w-full max-w-md">
                        <CardHeader className="space-y-1 flex flex-col gap-6">
                            <Image
                                src={buildLogosPath(isDark ? 'logo-dark.svg' : 'logo-light.svg')}
                                width={140}
                                height={140}
                                alt="Arkim logo"
                                className='m-auto'
                        ...
                        <CardContent className="space-y-4">
                                    ...
                                    companies.length > 0
                                        ...
                                        companies.map((company) => {
                                            return <button
                                                ...
                                                onClick={() =>
                                                    handleSelectCompany(company.companyId)
                                                }
                                                className={cn(
                                                    'w-full flex items-center justify-between p-4 border rounded-lg hover:bg-muted hover:border-primary transition-colors',
                                                    selectedCompanyId && company.companyId === selectedCompanyId ? 'bg-primary/10 border-primary' : ''
                                                )}
                                            >
                                        ...
                                        <div className='flex flex-col text-center items-center gap-4 mt-2 mb-5'>
                                            ...
                                            <div className='flex flex-col gap-1'>
                                                ...
                                                <p className='text-xs text-muted-foreground'>Please ask admin to assign you a company or else sign in with different account.</p>
                                            </div>
                                        </div>
                                }
                            </div>
                            ...
                            <div className="pt-4 border-t flex justify-center">
                                <Button
                                    variant="destructive"
                                    size='lg'
                                    onClick={() => { oidcAuthService.signOut(); setIsLoggingOut(true); }}
                                >
                                    Sign Out
                                </Button>
                            </div>
