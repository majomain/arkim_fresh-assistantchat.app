        ...
        const preColumns: ColumnDef<ThreadDetail>[] = [
            {
                ...
                cell: ({ row }) => (
                    <div className="text-start flex flex-row gap-1.5 items-center font-medium">
                ...
                cell: ({ row }) => {
                    ...
                    return (
                        <div className="text-center">
                            ...
                            >
                                <p
                                    className={`flex flex-row items-center gap-1 ${thread.status === 'open' ? 'text-primary' : thread.status === 'reported' ? 'text-success' : thread.status === 'closed' ? 'text-destructive' : 'text-success'}`}
                                >
                                    {thread.status
                                        ? thread?.status
                                              .charAt(0)
                    ...
                    {isDataLoading ? (
                        ...
                        table.getPaginationRowModel().rows.map((row) => (
                            <TableRow
                                ...
                                onClick={() =>
                                    router.push(
                                        `/thread?id=${row.original.threadId}`,
                                    )
                                }
                            >
