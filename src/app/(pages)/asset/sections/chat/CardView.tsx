...
export default function ChatCardView(
    ...
    return <div className="md:hidden">
        <CardWithPagination
            ...
            renderCard={(thread) =>
                <div key={thread.threadId} className="relative flex flex-col gap-4 border rounded-lg cursor-pointer">
                    {
                    ...
                    <div className="flex flex-col gap-3">
                        <div className="flex justify-between">
                            <Link href={`/thread?id=${thread.threadId}`} className="text-sm font-medium items-center w-full p-3">
                            ...
                            {processingThreads[thread.threadId] || isThreadProcessing(thread.threadId)
                                ...
                                thread.status === 'open'
                                    ?
                                    <ActionPopover action={action} threadId={thread.threadId} threadTitle={thread.title}>
                        ...
                        <p className="px-3 text-xs text-muted-foreground -mt-6">Created At: {getDateFromTimestamp(thread.createdAtUtc ?? '')}</p>

                        <Link href={`/thread?id=${thread.threadId}`} className="px-3 pb-3">
                            ...
                            <Badge variant='outline' className={`border-none ${thread.status === "open" ? "bg-primary/15" : thread.status === 'reported' ? "bg-success/15" : thread.status === 'closed' ? 'bg-destructive/15' : 'bg-success/15'}`}>
                                <p className={`text-xs ${thread.status === "open" ? "text-primary" : thread.status === 'reported' ? "text-success" : thread.status === 'closed' ? 'text-destructive' : 'text-success'}`}>
            ...
            emptyState={
                <div className="w-full flex flex-col items-center p-5 mt-5 gap-4">
                    <MessagesSquare className="text-muted-foreground w-15 h-15" strokeWidth={1} />
                    <div className="flex flex-col items-center">
                        <p className="text-base font-semibold">No Thread Found</p>
                        <p className="text-sm text-muted-foreground text-center">Create a new thread for your asset</p>
                    </div>
                </div>
            }
        />
