        ...
        const preColumns: ColumnDef<WorkOrderDetail>[] = [
            {
                ...
                cell: ({ row }) => <div className="text-start font-medium">{row.getValue('title')}</div>,
            },
            {
                ...
                cell: ({ row }) => <div>
                    <p className={cn(
                        ...
                        row.original.description.length > 100 && 'line-clamp-3'
                    )}>
                    ...
                    </p>
                    {
                        row.original.description.length > 100
                            ...
                            <Dialog>
                                <DialogTrigger>
                                    <span className={cn(
                                        row.original.description.length > 100 ? 'text-xs font-medium text-end text-link' : 'hidden'
                                    )}>show more</span>
                                </DialogTrigger>
                                <DialogContent>
                                    <DialogDescription />
                                    <DialogTitle>Description</DialogTitle>
                                    <p className='text-sm text-muted-foreground whitespace-pre-wrap'>{row.original.description}</p>
                                </DialogContent>
                            </Dialog>
                            :
                            null
            ...
            // },
            {
                ...
                cell: ({ row }) => <div className="text-center">{getDateFromTimestamp(row.getValue('dueDate') ?? '')}</div>,
            },
            {
                accessorKey: 'status',
                header: ({ column }) => <div className="flex justify-center">
            ...
            },
            {
                ...
                cell: ({ row }) => {
                    ...
                    return <div className='flex flex-col items-center gap-2'>
                        {
                            workOrder.threadId && workOrder.threadOpenedBy === user?.email && workOrder.status !== 'open'
                                ?
                                <Link href={`thread?id=${workOrder.threadId}`} className='text-xs font-medium border border-foreground hover:bg-muted hover:border-transparent rounded-md px-2 py-1.5 flex flex-row items-center gap-1'>
                                    <MessageSquare className='size-3.5' strokeWidth={2} />
                                    {workOrder.status === 'thread_opened' ? 'Continue Work' : 'View thread'}
                                </Link>
                                :
                                workOrder.status === 'open' && !workOrder.threadId
                                    ?
                                    <p className='text-xs font-medium border border-foreground hover:bg-muted hover:border-transparent rounded-md px-2 py-1.5 cursor-pointer flex flex-row items-center gap-1' onClick={() => {
                        ...
                        {
                            workOrder.attachments && workOrder.attachments.length ?
                        ...
                        <TableCell colSpan={columns.length} className="h-24">
                            <div className="text-center py-12">
                                ...
                                <p className="text-sm text-muted-foreground">
                                    Once you're assinged a work order, it will reflect here
                                </p>
                            </div>
                        </TableCell>
                    </TableRow>
