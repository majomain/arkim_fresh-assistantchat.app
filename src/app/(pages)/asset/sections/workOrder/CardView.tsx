...
export default function CardView({
    ...
    function handleCardClick(workOrder: WorkOrderDetail) {
        if (workOrder.threadId && workOrder.threadOpenedBy === user?.email && workOrder.status !== 'open') {
    ...
    return <div className="md:hidden">
        <CardWithPagination
            ...
            renderCard={(workOrder) => <div key={workOrder.id} className="w-full flex flex-col border rounded-md cursor-pointer">
                <div className="w-full pt-2 px-2 flex flex-row justify-between items-center" onClick={() => handleCardClick(workOrder)}>
                ...
                <div className="flex flex-col gap-2 py-5 px-2">
                    <p className={cn(
                        "text-sm font-medium",
                        ((workOrder.threadId && workOrder.threadOpenedBy === user?.email && workOrder.status === 'thread_opened') || (workOrder.status === 'open' && !workOrder.threadId)) && 'text-link'
                    ...
                    <div>
                        <p className={cn(
                            ...
                            workOrder.description.length > 100 && 'line-clamp-3'
                        )} onClick={() => handleCardClick(workOrder)}>
                            ...
                            workOrder.description.length > 100
                                ...
                                <Dialog>
                                    <DialogTrigger onClick={(e) => e.stopPropagation()}>
                                        <span className={cn(
                                            workOrder.description.length > 100 ? 'text-xs font-medium text-end text-link' : 'hidden'
                                        )}>show more</span>
                                    </DialogTrigger>
                ...
                <div className="flex flex-col items-center gap-2 pb-2 px-2 w-full">
                    {
                        workOrder.attachments && workOrder.attachments.length && <AttachmentsDialog workOrder={workOrder} fullWidth={true}>
                            <Button variant='secondary' className="w-full">
                                <Image className='size-3.5' strokeWidth={2} />
                                Attachments
                            </Button>
                        </AttachmentsDialog>
                    }
                    {
                        workOrder.workLogs && workOrder.workLogs.length && <WorkLogDialog workOrder={workOrder}>
                            <Button variant='secondary' className="w-full">
                                <Logs className='size-3.5' strokeWidth={2} />
                                Work Logs
                            </Button>
                        </WorkLogDialog>
                    }
...

                {
                    workOrder.status === 'thread_opened' && workOrder.threadOpenedBy !== user?.email
                        ?
                        <Badge variant='secondary' className="mb-3 mx-auto" onClick={() => handleCardClick(workOrder)}>
            ...
            emptyState={
                <div className="w-full flex flex-col items-center p-5 mt-5 gap-4">
                    <ClipboardList className="text-muted-foreground w-15 h-15" strokeWidth={1} />
                    <div className="flex flex-col items-center">
                        <p className="text-base font-semibold">No Work Order Found</p>
                        <p className="text-sm text-muted-foreground text-center">Works Orders assigned to you will show here</p>
                    </div>
                </div>
            }
        />
