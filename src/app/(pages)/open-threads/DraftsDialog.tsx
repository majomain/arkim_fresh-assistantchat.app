...
import { Box, Edit, Layers2, MessageSquare, SendHorizontalIcon, Trash2, X } from "lucide-react"
    ...
    return (
        <AlertDialog>
            ...
            <AlertDialogContent>
                ...
                <AlertDialogDescription>Are you sure you want to delete all the drafts?</AlertDialogDescription>
                <AlertDialogFooter>
    ...
    return (
        <AlertDialog>
            ...
            <AlertDialogContent>
                ...
                <AlertDialogDescription>Are you sure you want to delete draft for <b>{title}</b>?</AlertDialogDescription>
                <AlertDialogFooter>
                    <AlertDialogCancel>Cancel</AlertDialogCancel>
                    <AlertDialogAction asChild onClick={onDelete}>
    ...
    return (
        <AlertDialog>
            ...
            <AlertDialogContent>
                ...
                <AlertDialogDescription>Are you sure you want to send message from the draft for <b>{title}</b>?</AlertDialogDescription>
                <AlertDialogFooter>
                    <AlertDialogCancel>Cancel</AlertDialogCancel>
...
export default function DraftsDialog() {
    ...
    const handleClearAll = () => {
        clearAllDrafts();
        successToast({ title: "Drafts deleted", description: "All the drafts have been deleted successfully." });
    };

    const handleDeleteDraft = (draftId: string, showToast = true, deleteAttachment = true) => {
        const parsed = parseDraftKey(draftId);
        if (!parsed) return;
        parsed.type === 'thread' ? clearThreadDraft(parsed.id, deleteAttachment) : clearAssetDraft(parsed.id, deleteAttachment);
        if (showToast) successToast({ title: "Draft deleted", description: "The draft has been deleted successfully." });
    };
    ...
    const handleEditDraft = (draftId: string) => {
        ...
        if (!parsed) return;
        setOpen(false);
        router.push(parsed.type === 'thread' ? `/thread?id=${parsed.id}` : `/asset?id=${parsed.id}`);
    };
    ...
    const handleSendDraft = (draftId: string, entry: DraftEntry) => {
        ...
        try {
            if (parsed.type === 'thread') {
                postMessageAsync(entry.text, entry.assetId ?? '', parsed.id, false, entry.title, undefined, 'user', entry.attachmentUrls);
                router.push(`/thread?id=${parsed.id}`);
            } else {
                postMessageAsync(entry.text, parsed.id, '', false, '', undefined, 'user', entry.attachmentUrls);
            }
            handleDeleteDraft(draftId, false, false);
            setOpen(false);
        } catch {
            errorToast({ title: "Error", description: "Failed to send the draft. Please try again." });
        }
    };
                        ...
                        >
                            ...
                            {hasDrafts ? (
                                <>
                                    <div className="flex flex-col gap-5 overflow-y-auto px-6 py-4">
                                        {draftEntries.map(([draftId, entry]) => {
                                            ...
                                            return (
                                                <Card key={`${id}-${type}`} className="p-5 gap-5 !bg-transparent !shadow-none">
                                                    <CardHeader className="p-0 lg:flex lg:flex-row lg:items-center lg:justify-between">
                                                        <div className="lg:w-full flex items-center gap-2">
                                                            <div className="bg-muted/50 p-2.5 rounded-full">
                                                                {type === 'thread' ? <MessageSquare className="size-5" /> : <Box className="size-5" />}
                                                            </div>
                                                            <div>
                                                        ...
                                                        <div className="lg:flex gap-2 w-full justify-end hidden">
                                                            <TooltipIconButton variant="secondary" onClick={() => handleEditDraft(draftId)} tooltip="Edit draft" className="size-9">
                                                                <Edit />
                                                            </TooltipIconButton>
                                                            {
                                                                entry.text && <SendConfirmation title={entry.title} onSend={() => handleSendDraft(draftId, entry)} disabled={!entry.text.trim() && entry.attachmentUrls.length === 0} />
                                                            }
                                                            <DeleteConfirmation title={entry.title} onDelete={() => handleDeleteDraft(draftId)} />
                                                        </div>
                                                    </CardHeader>
                                ...
                                <div className="flex flex-col items-center justify-center gap-3 py-20">
                                    <Layers2 className="size-15 text-muted-foreground" strokeWidth={1.2} />
