...
export type ReasoningType = Partial<
    Record<Tags, { title: string; content: string }>
>;
...
export class StreamlineChatHandler {
    ...
    public handleDeltaEvent(data: DelteEventData) {
        ...
        if (
            data.type === 'response' &&
            (this.currentStep === 'routing' || data.step === 'profile_answer')
        ) {
            this.options.appendResponse(data.delta);
        } else {
            this.options.updateThinking(
                this.currentStep,
                this.currentTag,
                data.delta,
            );
