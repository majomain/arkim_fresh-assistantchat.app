...
export default function ThreadSelectiveChat({
    ...
    // runtime thread data
    const { currentThreadId, isThreadProcessing, closeThread, reportThread } =
        useThread();

    // runtime chat data
    const { processingThreads, processedThreads, removeProcessedThread } =
        useChat();
    ...
    return (
        threads.length > 0 && (
            <div
                className={clsx(
                    'flex flex-col items-stretch gap-1 border-l ml-2.5 textarea-slide-down',
                    hidden && 'hidden',
                )}
            >
                {threads.map((thread) => {
                    return (
                        <div
                            ...
                            className={clsx(
                                ...
                                {
                                    'bg-sidebar-accent':
                                        thread.threadId === currentEllipsis,
                            ...
                            <div className="w-full flex flex-row items-center justify-between">
                                <Tooltip delayDuration={600}>
                                    <TooltipTrigger asChild>
                                        <p
                                            className="py-2 pl-2 pr-2.5 text-start flex-1 truncate"
                                            onClick={() => {
                                ...
                                {((processingThreads[thread.threadId] &&
                                    ...
                                    <Loader2
                                        className="size-4 animate-spin mr-2.5"
                                        strokeWidth={3}
                                    />
                                )}
