...
export default function ThreadSearch({ isTyping, search, closeDialog }: { isTyping: boolean; search: string; closeDialog: () => void; }) {
    // asset utils
    ...
    return isLoading || isAssetListLoading || isTyping ? (
        ...
        <div className="grid grid-cols-1 gap-2 max-h-[60dvh] overflow-y-auto px-2 scrollable">
            {threads.map((thread, index) => {
                ...
                return <Card key={thread.threadId} className="cursor-pointer hover:shadow-none" onClick={() => {
                    closeDialog();
