...
export default function RecentWorkOrders() {
    ...
    const [recentWorkOrders, setRecentWorkOrders] = useState<WorkOrderDetailList>([]);
    const [loadingRecentWorkOrders, setLoadingRecentWorkOrders] = useState<boolean>(true);
    ...
    const broadcast = useThreadBroadcast((event) => {
        if (event.type === 'THREAD_STATUS_UPDATED') {
            setRecentWorkOrders((prev) => prev.filter((workOrder) => workOrder.threadId !== event.threadId));
        }
    });
    ...
    async function getRecentWorkOrders() {
        try {
            ...
            const data = await workOrderService.getAssignedWorkOrders(null, selectedLocation?.id, null);
            setRecentWorkOrders(data);
        } catch (error: any) {
            errorToast({ title: 'Recent work order fetch error', description: error.message });
        } finally {
            setLoadingRecentWorkOrders(false);
        }
    }

    const allItems = useMemo(() => recentWorkOrders.filter((workOrder) => (workOrder.threadOpenedBy === user?.email || workOrder.threadOpenedBy === null) && (workOrder.status !== 'completed' && workOrder.status !== 'cancelled')).slice(0, 15), [recentWorkOrders]);

    // Reset to first page when viewport or data changes
    useEffect(() => { setPage(0); }, [visibleSlides, allItems.length]);

    useEffect(() => {
        if (!isLoadingLocations && selectedLocation) getRecentWorkOrders();
    ...
    return <>
        <div className="flex items-center justify-between" >
            ...
            <div className="flex flex-row gap-5 items-center">
                {!loadingRecentWorkOrders && !isAssetListLoading && totalPages > 1 && (
...

                {
                    recentWorkOrders.length > 15
                        ?
                        <Link href='/work-orders?status=thread_opened' className="text-link font-semibold text-sm">View All</Link>
                        :
                        null
                }
            </div>
            ...
            {loadingRecentWorkOrders || isAssetListLoading ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-2.5 content-start">
                    {visibleItems.map((workOrder, index) => {
                        return <DisplayCard key={`workorder-${workOrder.id}-${index}`} workOrder={workOrder} getStatusLabel={getStatusLabel} />
                    })}
                </div>
            ) : (
                <div className="w-full flex flex-col justify-center items-center">
                    <ClipboardListIcon className="size-15 text-muted-foreground" strokeWidth={1} />
                    <div className="flex flex-col gap-1 items-center justify-center text-center">
                        <p className="text-base font-semibold">No recent work orders found</p>
                        <span className="text-sm text-muted-foreground">Start working on the pending work orders</span>
                    </div>
                </div>
            )
            }
