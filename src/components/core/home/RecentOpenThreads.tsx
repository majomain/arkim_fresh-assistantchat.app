...
export default function RecentOpenThreads() {
    ...
    const [recentOpenThreads, setRecentopenThreads] =
        useState<ThreadDetailList>([]);
    const [loadingRecentOpenThreads, setLoadingRecentOpenThreads] =
    ...
    async function getRecentOpenThreads() {
        try {
            ...
            const data = await messagingService.getOpenThreads(
                selectedLocation?.id ?? '',
            );
            setRecentopenThreads(data);
        } catch (error: any) {
            errorToast({
                title: 'Recent open thread fetch error',
    ...
    const allItems = useMemo(
        () =>
            recentOpenThreads
                .filter((openThread) => !openThread.workOrderId)
                .slice(0, 15),
        [recentOpenThreads],
    );
                ...
                {loadingRecentOpenThreads || isAssetListLoading ? (
                    ...
                    <div className="w-full flex flex-col justify-center items-center">
                        ...
                        <div className="flex flex-col gap-1 items-center justify-center text-center">
                            <p className="text-base font-semibold">
                                No recent open thread found
                            </p>
                            <span className="text-sm text-muted-foreground">
                                Start working on the pending work orders or
