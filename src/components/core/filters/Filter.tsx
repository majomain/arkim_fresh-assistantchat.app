...
export default function Filter({
    ...
    // current tab flag
    const [currentTab, setCurrentTab] = useState<FilterTabType['value']>(filterTabs[0]?.value || '');
    ...
    return <DropdownMenu open={open} onOpenChange={setOpen}>
        <DropdownMenuTrigger asChild className={cn(
            ...
            <Button variant='outline'><FunnelIcon /> {!isMobile && 'Filters'}</Button>
        </DropdownMenuTrigger>

        <DropdownMenuContent align="start" className="bg-card w-70 overflow-hidden">
                    ...
                    {filterTabs.map(ft => (
                        <p
                            ...
                            className={cn(
                                ...
                                currentTab === ft.value ? 'text-primary' : 'hover:border-border'
                            )}
                ...
                <div className="p-2 overflow-auto h-40 w-full">
                    {filters.map(filter => {
                        ...
                        if (filter.type === 'list' && filter.options) {
                            return (
                                <div key={filter.tab} className="flex flex-col gap-2">
            ...
            <div className="border-t-1 pb-2 pt-4 px-2 flex flex-row items-center justify-end gap-4">
                <Button size='sm' variant='success' onClick={() => { applyFilter(); setOpen(false); }}>Apply</Button>
                <Button size='sm' variant='secondary' onClick={() => { clearFilter(); setOpen(false); setCurrentTab(filterTabs[0].value) }}>Clear All</Button>
            </div>
        </DropdownMenuContent>
    </DropdownMenu>;
}
