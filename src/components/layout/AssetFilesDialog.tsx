...
export default function AssetFilesDialog({ children }: { children: React.ReactNode; }) {
    ...
    async function getDocuments() {
        try {
            if (selectedLocation && currentAsset) {
                ...
                const response = await onboardingService.getDocumentsByAssetAndModelId(currentAsset.id, currentAsset.assetModelId ?? '');
            ...
            <div className='max-h-100 overflow-y-auto p-2 flex flex-col gap-5'>
                {
                    isDataLoading
                        ?
                        Array.from({ length: 2 }).map((_, index) => <div key={index} className="w-full flex flex-row justify-between items-center p-2 border rounded-md">
                        ...
                        documentList.length
                            ?
                            documentList.map((doc) => <div key={doc.documentId}
                                onClick={() => setPreviewData({ url: doc.previewUrl, name: doc.fileName })}
                                ...
                                <div className="w-full flex flex-col gap-5">
                                    ...
                                    <div className="flex flex-row gap-2 items-center text-sm font-medium text-muted-foreground">
                                        ...
                                        <p>Source: {doc.source.charAt(0).toUpperCase() + doc.source.slice(1)}</p>
                                    </div>
                                </div>
                            </div>
                            )
                            ...
                            <div className="flex flex-col items-center p-2 gap-4">
                                <Files className="text-muted-foreground size-15" strokeWidth={1.5} />
                                <div className="flex flex-col items-center">
                                    <p className="text-lg font-bold">No Document Found</p>
                                    <p className="text-sm text-muted-foreground text-center">Documents will show up once it is ingested</p>
                                </div>
                            </div>
                }
