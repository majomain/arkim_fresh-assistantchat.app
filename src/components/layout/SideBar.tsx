        ...
        <Sidebar variant="sidebar" collapsible="icon">
            ...
            <SidebarContent>
                        ...
                        <SidebarMenu>
                            ...
                            <SidebarMenuItem>
                                ...
                                >
                                    <div className='cursor-pointer'>
                                        ...
                                        <span className="group-data-[collapsible=icon]:hidden text-foreground">New Asset</span>
                                    </div>
                                </SidebarMenuButton>
                            </SidebarMenuItem>
                            ...
                            {SideBarMenu.map((item) => {
                                ...
                                return <SidebarMenuItem key={item.href}>
                                    <SidebarMenuButton isActive={pathName.includes(item.href)} tooltip={item.title} asChild>
                                        <Link
                                            onClick={() => {
                                                if (isMobile) toggleSidebar();
                                            }}
                                        ...
                                        >
                                            <div className='flex flex-row items-center gap-2'>
                                                ...
                                                <span className="group-data-[collapsible=icon]:hidden text-foreground">{item.title}</span>
                                            </div>

                                            {
                                                draftCount > 0 && item.title.toLowerCase().includes('draft') && !pathName.includes(item.href)
                                                    ?
                                                    <span className='text-xs bg-sidebar-primary text-sidebar-primary-foreground px-1.5 py-0.5 rounded-full'>{draftCount}</span>
                                                    :
                                                    null
                                            }
                                        </Link>
                ...
                <div className={cn(
                    ...
                    <Tooltip>
                        <TooltipTrigger>
                            <LucideIcons.RefreshCcw className={cn(
                ...
                <SidebarMenu>
                    <SidebarMenuItem>
                        <SidebarMenuButton tooltip='Back to Company'>
                            <Link href='/company-select' className='flex items-center gap-2 w-full'>
